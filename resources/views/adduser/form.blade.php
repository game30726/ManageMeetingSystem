        ...
        <div class="card-body">
            <div class="col-12">
                <table class="table table-bordered table-hover" id="tab_logic">
                        <thead>
                            <tr>
                            <th style="border : 0px" class="text-center"><h5>เพิ่มประธานในที่ประชุม</h5></th>
              ...
              <table>
                    <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                            <form action="{{ route('adduser.store'), $user->id }}" method="POST">
                                @method('PUT')
                                @csrf
                                <input type="hidden" name="user_id" value="{{$user->id}}">
                                <td width="750">{{$user->name}}</td>
                                <td>
                                <button type="submit" class="btn btn-success">เพิ่มผู้เข้าร่วม</button>
                                </td>
                            </form>
                              </tr>
                              @endforeach
